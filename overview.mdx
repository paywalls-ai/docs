...
- **Validates** the user’s connection and balance
...
<Note>
  **Each API key is tied to a single provider**\
  However, every request can target any model that provider supports, with dynamic pricing per model.
</Note>
...
| **User Endpoints** (`/user/*`)           | Tools to manage paywall connection, top-up, balance lookup, and manual charges.                                          |
...
| **Connection Logic**                     | Ensures only connected (authorized) users can be charged. Returns a connect link if not connected.                       |
...
4. **Connection & balance check**
   - Not connected → return `connect` link
   - Low balance → return `topup` link
   - Connected \+ funded → proceed
