...
See also: `more/errors-limits-reliability.mdx` (Idempotency) and `how-to-guides/connect-stripe.mdx`.
...
- Encryption at rest: data at rest is encrypted in our managed infrastructure.

Refer to: `privacy.mdx` for full privacy policy and data subject rights.
...
See: `how-to-guides/connect-stripe.mdx` and `more/test-keys-and-environments.mdx`.

## Network & transport security

- All API endpoints are served over HTTPS; reject plain HTTP.
- Consider outbound egress controls on your infrastructure to prevent accidental data exfiltration.
...
- Default mode: use a Restricted Stripe key; store Stripe `payment_intent`/`checkout_session` ids in ledger metadata for reconciliation. Do not expose Stripe secrets in clients.
- Shared mode: treat assistant messages that include authorization or top‑up links as normal model replies; render them directly. No keys are required on the client beyond your paywall key on the server.

Cross‑refs: `core-concepts/user-identity.mdx`, `core-concepts/paywalls-wallets-ledger.mdx`, `core-concepts/pricing-metering.mdx`.

## Your responsibilities

- Keep secrets server‑side; never expose paywall or provider keys to browsers or untrusted clients.
