...
> Security: Use a **restricted** key with only the permissions Paywalls needs; keep all API keys server‑side. Test with Stripe test keys first, then swap to live—no code changes required.
