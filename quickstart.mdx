...
<Info>**OpenAPI spec:** https://api.paywalls.ai/v1/openapi.yml</Info>
...
<Note>
  **Note:** You can pass the user ID in the request body (`user` property) or in
  an `X-Paywall-User` header. [Learn more about User
  Identification](/paywall/user-identity).
</Note>

## 4. How It Works (In Short)

1. **Proxy receives your request**\
   Identifies the user and checks their paywall authorization and balance.
2. **If not authorized** → returns an `authorization` link\
