...
3. **User state verified**:
   - If not connected → return `connected: false` \+ `connect` link.
   - If connected, check balance:
     - Insufficient balance → return `connected: true` \+ `topup` link.
...
- **Integrate once**: No changes needed on the frontend. Just point requests to `https://paywalls.ai/api/v1/chat/completions` instead of `api.openai.com`.
...
- **Identify the user**:
  - For **chat completions**, include the user’s ID either:
...
- **Track usage**: Monitor charges, user sessions, and connection status through API responses or optional integrations (e.g., webhooks—coming soon).
