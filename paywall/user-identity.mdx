...
<Warning>Avoid using spaces and special characters in the user ID.</Warning>
...
2. **Authorization check**:
   - If the user is **not authorized** → response includes `{ connected: false, connect_link: "..." }`
   - No charge is applied.
3. **Balance check**:
   - If **authorized but no balance** → response includes `{ connected: true, balance: 0, topup_link: "..." }`
   - No charge is applied.
